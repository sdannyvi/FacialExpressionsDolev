...
parser.add_argument("--layer", type=str, default="fc1",
                    help="the layer in which the embeddings will be extracted from. "
                         "can get one of the following values: fc1, fc2, fc3.")
...
# create hook function to get embeddings
hook_outputs = {}
...
# extract embeddings
def get_embeddings(image_path, model, transforms,layer):
    # get pil image and process it
    pil_image = Image.open(image_path)
    input_image = transforms(pil_image).unsqueeze(0)
    input_image = input_image.to(accelerator.device)
    with torch.no_grad():
        model_output = model(input_image)
    # get the layers output
    embeddings = hook_outputs[layer]
    embeddings = embeddings.squeeze(0).cpu()
    return embeddings

# load, transform images, get embeddings
...
knowledge_base_set['embedding'] = knowledge_base_set['file_path'].apply(lambda x: get_embeddings(x, resEmotenet_model, transform, layer))
...
# else, create predictions column and copy full test set
    ...
    results_df = results_df.assign(predictions=None, top_example_1=None, top_example_2=None, top_example_3=None,
                                   ...
                                   top_example_3_similarity=None, query_file_path=None)
...
for curr_batch in range(num_batches):
    ...
    for _, row in batch_df.iterrows():
        ...
        query_embedding = get_embeddings(image_path, resEmotenet_model, transform, layer)
