...
#
#  Run with:
#      mkdir -p experiments/oracle/runs/rules_direct_zs_rag1_rag2
#      sbatch experiments/oracle/runs/rules_direct_zs_rag1_rag2/rules_direct_zs_rag1_rag2.sh
#
#  TWO THINGS TO SET:
...
#SBATCH --job-name=rules_direct_zs_rag1_rag2
#SBATCH --output=/truenas/home/sdolev/FacialExpressionsDolev/experiments/oracle/runs/rules_direct_zs_rag1_rag2/%x_%j.log
...
EXP_GROUP="oracle/runs/rules_direct_zs_rag1_rag2"
...
PIPELINE_ARGS=(
  # the oracle split of the validation set (split_merge_oracle.py split): every sample has the same wrong
  # top-1 and top-2 label, so only the oracle gate sends it to the framework
  --test_path    "$PROJECT/experiments/oracle/subst_to_test/ferplus_val_both_wrong_ungated.csv"
  ...
  # the oracle gate: every sample that is not High retrieval goes to the framework, so every sample of the
  # split above does; with the default gate they would all run the original RAG
  --gate         oracle
