...
PIPELINE_ARGS=(
  # the FULL RAF-DB test set: rag.py routes every sample itself, so there is no split/merge step.
  ...
  # the RAF-DB train set as the knowledge base
  --knowledge_base_path "$PROJECT/data/raf-db/train_test_sets/train_set.csv"
  # retrieval: CLIP embeddings reduced by LDA. Both are the defaults; passed explicitly so the
