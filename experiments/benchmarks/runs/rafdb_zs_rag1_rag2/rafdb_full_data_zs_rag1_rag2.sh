...
#
#  Run with:
#      mkdir -p experiments/benchmarks/runs/rafdb
#      sbatch experiments/benchmarks/runs/rafdb/rafdb_full_data_zs_rag1_rag2.sh
#
#  TWO THINGS TO SET:
...
#SBATCH --job-name=rafdb_full_data_zs_rag1_rag2
#SBATCH --output=/truenas/home/sdolev/FacialExpressionsDolev/experiments/benchmarks/runs/rafdb/%x_%j.log
...
EXP_GROUP="benchmarks/runs/rafdb"
...
PIPELINE_ARGS=(
  # the full RAF-DB test set; the gate sends samples with different top-1 and top-2 labels
  # to the framework and the rest to the original RAG
  --test_path    "$PROJECT/data/raf-db/train_test_sets/test_set.csv"
  ...
  # the full RAF-DB training set as the knowledge base
