...
#
#  Run with:
#      mkdir -p experiments/benchmarks/runs/affectnet_zs_rag12
#      sbatch experiments/benchmarks/runs/affectnet_zs_rag12/affectnet_full_data_zs_rag12.sh
#
#  TWO THINGS TO SET:
...
#SBATCH --job-name=affectnet_zs_rag12
#SBATCH --output=/truenas/home/sdolev/FacialExpressionsDolev/experiments/benchmarks/runs/affectnet_zs_rag12/%x_%j.log
...
PIPELINE_ARGS=(
  # only the gated rows of the AffectNet validation set, which is the test set (split_merge_gated.py
  # split): every sample here has different top-1 and top-2 labels, so the gate sends all of them to
  # the framework. The remaining test samples keep their original RAG predictions and are merged back
  # afterwards
  --test_path    "$PROJECT/experiments/benchmarks/gated_test_sets/affectnet_gated_test.csv"
  ...
  # the full AffectNet train set as the knowledge base - the same one the original RAG run used
