...
PIPELINE_ARGS=(
  # the FULL AffectNet validation set, which is the test set: rag.py routes every sample itself, so
  # there is no split/merge step. Samples whose top-1 and top-2 labels agree run the original RAG,
  # the rest run the framework
