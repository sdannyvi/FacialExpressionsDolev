...
#
#  Run with:
#      mkdir -p experiments/benchmarks/runs/ferplus_zs_rag12
#      sbatch experiments/benchmarks/runs/ferplus_zs_rag12/ferplus_zs_rag12.sh
#
#  TWO THINGS TO SET:
...
#SBATCH --job-name=ferplus_zs_rag12
#SBATCH --output=/truenas/home/sdolev/FacialExpressionsDolev/experiments/benchmarks/runs/ferplus_zs_rag12/%x_%j.log
...
EXP_GROUP="benchmarks/runs/ferplus_zs_rag12"
...
PIPELINE_ARGS=(
  # the full FER+ private test set (50% vote threshold); the gate sends samples with different
  # top-1 and top-2 labels to the framework and the rest to the original RAG
  --test_path    "$PROJECT/rag_thresholds/train_test_sets/private_test_50%.csv"
  ...
  # the FER+ training set (50% vote threshold) as the knowledge base - the plain one, not the
  # cleaned kb of the ablation
  --knowledge_base_path "$PROJECT/rag_thresholds/train_test_sets/kb_50%.csv"
