...
#
#  Run with:
#      mkdir -p experiments/top_k_ablation/runs
#      sbatch experiments/top_k_ablation/runs/top_k_job.sh
#
#  TWO THINGS TO SET:
...
#SBATCH --job-name=rag_gemma_4_31b_no_thinking_top_k_3
#SBATCH --output=/truenas/home/sdolev/FacialExpressionsDolev/experiments/top_k_ablation/runs/%x_%j.log
...
EXP_GROUP="top_k_ablation/runs"

# Which pipeline: zero_shot | rag
PIPELINE="rag"

# How many retrieved neighbours the generator sees. Overridable at submit time so
# the top-k ablation runs off this ONE file - keep the job name and K in step:
#     for k in 3 4 5; do
#       sbatch --job-name=rag_gemma_4_31b_no_thinking_top_k_$k \
#              --export=ALL,TOP_K=$k \
#              experiments/top_k_ablation/runs/top_k_job.sh
#     done
# Bare `sbatch top_k_job.sh` (no --export) falls back to the default below.
TOP_K="${TOP_K:-3}"
